...
* `splint.cljs` -- open or create a Splint terminal and run [Splint](https://github.com/NoahTheDuke/splint) on the current file. By default, this assumes you have [Babashka](https://github.com/babashka/babashka) installed as it uses `bb` to run Splint. You can modify the script to use the Clojure CLI instead -- an example `splint-command` is provided that would run `clojure -M:splint`, assuming you have `:splint` as an alias (which my [dot-clojure](https://github.com/seancorfield/dot-clojure) repo provides).
